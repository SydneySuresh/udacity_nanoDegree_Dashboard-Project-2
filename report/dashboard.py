...
class ReportDropdown(Dropdown):
    ...
    def component_data(self,entity_id, model):
        # Use the model argument to call the employee_events method
...
class BarChart(MatplotlibViz):
    ...
    def visualization(self, entity_id, model):
        ...
        output = self.predictor.predict_proba(df)
        
        # Index the second column of predict_proba output
        # The shape should be (<number of records>, 1)
        df = pd.DataFrame(output, columns=['Class_0', 'Class_1'])
        df = df.iloc[:, [1]] 
        ...
        if model.name == 'team':
            pred = df.mean().values[0]  # Extract scalar mean value
        else:
            pred = df.iloc[0, 0]  # First value of the second column
...
def get_employee(employee_id):
    ...
    employee = Employee.username(employee_id)
